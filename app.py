...
def api_osint_brechas():
    """Wrapper for external breach scan with SQLite result caching using ConsultaOsint and CacheConsulta."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
    ...
    # 1. Ensure HaveIBeenPwned FuenteOsint exists
    ...
    # 3. Hit HIBP API (or fallback to mockup list)
...
# ── Nuevos endpoints: intel.db / osint.db / ETL ──────────────────────────────

@nexo.route("/api/intel/entidades", methods=["GET"])
@login_required
def api_intel_entidades():
    """Aggregates all entity types from intel.db."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
    ...
    return jsonify({
        ...
        "alias": [{
            "id": a.id, "valor": a.valor, "contexto": a.contexto or "Sin contexto",
        } for a in alias_list],
        "telefonos": [{
        ...
        "organizaciones": [{
            "id": o.id, "nombre": o.nombre or "Sin nombre", "tipo": o.tipo or "",
            "descripcion": o.descripcion or "", "activa": o.activa,
        } for o in organizaciones],
        "vehiculos": [{
            "id": v.id, "placa": v.placa or "No registra", "tipo": v.tipo or "",
            "marca": v.marca or "", "modelo": v.modelo or "",
            "anio": v.anio, "color": v.color or "",
        } for v in vehiculos],
        "cuentas": [{
            "id": c.id, "numero": c.numero or "", "tipo": c.tipo or "",
...
def api_intel_hallazgos():
    """Returns intelligence findings from the intel bind."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
        return jsonify({"error": "Acceso restringido."}), 403
...
@login_required
def api_intel_grafo():
    """Returns graph nodes and edges from the intel bind (Data Warehouse layer)."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
        return jsonify({"error": "Acceso restringido."}), 403
...
def api_osint_indicadores():
    """Returns active risk indicators from the OSINT bind."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
        return jsonify({"error": "Acceso restringido."}), 403
    ...
    return jsonify([{
        "id": i.id, "tipo": i.tipo or "", "valor": i.valor,
        "descripcion": i.descripcion or "", "nivel_riesgo": i.nivel_riesgo or "Bajo",
...
@login_required
def api_etl_status():
    """Computes ETL pipeline metrics from cross-DB record counts."""
    role = session.get("role")
    if role not in ["admin", "analista", "director"]:
        return jsonify({"error": "Acceso restringido."}), 403
